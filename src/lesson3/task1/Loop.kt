...
fun squareSequenceDigit(n: Int): Int {
    ...
    while (number < n) {
        counter++
        number += digitNumber(counter * counter)
    }
    sqr = counter * counter
    (n until number).forEach { sqr /= 10 }
    return sqr % 10
}
/**
 * Сложная
...
fun fibSequenceDigit(n: Int): Int {
    ...
    while (number < n) {
        counter++
        number += digitNumber(fib(counter))
    }
    fib = fib(counter)
    (n until number).forEach { fib /= 10 }
    return fib % 10
}
